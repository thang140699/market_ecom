...
export const myOrders = () => async (dispatch) => {
  try {
    ...
    dispatch({ type: MY_ORDERS_SUCCESS, payload: data.ordersSystem });
  } catch (error) {
    dispatch({
      type: MY_ORDERS_FAIL,
      payload: error.response.data.message,
...
export const getOrderSystem = () => async (dispatch) => {
  try {
    dispatch({ type: ALL_ORDERS_REQUEST });

    const { data } = await axios.get("/api/v1/admin/ordersSystem");

    dispatch({ type: ALL_ORDERS_SUCCESS, payload: data.ordersSystem });
  } catch (error) {
    dispatch({
